...
#### 3.7.2 讲讲 AtomicInteger 的使用

#### 3.7.3 AtomicInteger类的原理 (UnSafe)

### 3.8 线程池
#### 3.8.1 Java线程池原理, 设置线程数一般基于哪些考虑

#### 3.8.2 为什么要用线程池

#### 3.8.3如何创建线程池

#### 3.8.4 实现Runnable接口和Callable接口的区别

#### 3.8.5 执行execute()方法和submit()方法的区别是什么呢？

#### 3.8.6 Latch, Barrier, Semaphore使用场景

#### 3.8.7 Future, FutureTask, CompletionService的使用(JUC的了解)
